...
class QuestionsView(arcade.View):
    def __init__(self):
        ...
        # את הפעלת המנהל נעשה ב-on_show_view
        ...
        # התיקון: שימוש ב-UIAnchorLayout במקום UIAnchorWidget
        anchor = arcade.gui.UIAnchorLayout()
        anchor.add(child=self.v_box, anchor_x="center_x", anchor_y="center_y")
        self.manager.add(anchor)

    def on_show_view(self):
        """נקרא אוטומטית כשהמסך עולה"""
        self.manager.enable()
        arcade.set_background_color(arcade.color.ARMY_GREEN)
        # עכשיו בטוח לבקש שאלה מה-AI, כי המסך כבר מוכן
    ...
    def send_answer_and_get_next(self):
        ...
        # מכיוון שזה לוקח זמן, במסך אמיתי כדאי לעשות את זה ב-thread נפרד
        new_ai_response = ask_ollama(prompt)
        self.question_text.text = new_ai_response

    def on_hide_view(self):
        """מכבה את ה-UI כשעוברים מסך"""
